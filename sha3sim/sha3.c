...
sha3_return_t
sha3Init(void *priv, unsigned bitSize) {
...
void
sha3Init256(void *priv)
{
  sha3Init(priv, 256);
}

void
sha3Init384(void *priv)
{
  sha3Init(priv, 384);
}

void
sha3Init512(void *priv)
{
  sha3Init(priv, 512);
}

enum SHA3_FLAGS
sha3SetFlags(void *priv, enum SHA3_FLAGS flags)
{
  sha3_context *ctx = (sha3_context *) priv;
 ...
 */
void
sha3Update(void *priv, void const *bufIn, size_t len)
{
  sha3_context *ctx = (sha3_context *) priv;
  ...
  if(old_tail) {              /* will have one word to process */  // 开始一个字的计算
  ...
  /* now work in full words directly from input */  // 现在直接从输入中使用完整的字
...
/* This is simply the 'update' with the padding block.  // 这是带有填充块的“更新”
 * The padding block is 0x01 || 0x00* || 0x80. First 0x01 and last 0x80 
 * bytes are always present, but they can be the same byte.
 */
void const *
sha3Finalize(void *priv)
{
  sha3_context *ctx = (sha3_context *) priv;
  ...
  // 根据SHA-3规范，附加2位后缀“01”。我们在下面使用 1<<2 代替1作为填充。下面的 0x02 对应于后缀 01 。总的来说，我们输入0，然后输入1，最后输入1来开始填充。如果没有 M||01 ，我们将简单地使用1来开始填充。
...
sha3_return_t sha3HashBuffer( unsigned bitSize, enum SHA3_FLAGS flags, const void *in, unsigned inBytes, void *out, unsigned outBytes ) {
  sha3_return_t err;
  sha3_context c;

  err = sha3Init(&c, bitSize);
  if( err != SHA3_RETURN_OK )
    return err;
  if( sha3SetFlags(&c, flags) != flags ) {
    return SHA3_RETURN_BAD_PARAMS;
  }
  sha3Update(&c, in, inBytes);
  const void *h = sha3Finalize(&c);
